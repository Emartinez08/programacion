...
signal_connect(Grp, :clicked) do widget
    ...
    # Verificar si 'sol' contiene datos
    println("Solving results: ", sol)

    # Verificar si las dimensiones de 'sol' son correctas
    if !isempty(sol.t) && size(sol)[1] > 0
        # Graficar los resultados
        p1 = plot(sol.t, sol[1, :], color="red", label="Hemicellulose")
        plot!(sol.t, sol[2, :], color="blue", label="Xylose")
        plot!(sol.t, sol[3, :], color="green", label="Furfural")

        title!("Concentration Profiles")
        xlabel!("Residence Time [min]")
        ylabel!("Concentration [g/L]")

        # Aquí se grafica en el Canvas (o área de dibujo)
        display(p1)  # Asegúrate de que la visualización esté activa

        # Actualizar el canvas para reflejar los cambios
        Gtk.update!(c2)  # Asegúrate de que el canvas se actualice con la nueva gráfica

        # Mostrar el canvas dentro del contenedor
        set_gtk_property!(c2, :visible, true)
    else
